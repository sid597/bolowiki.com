...
class queryDb():
    ...
    def getUserData(self):
        self.c.execute("SELECT * FROM users WHERE username = (%s)",
                  (thwart(self.username),))

    def getUserId(self):
        self.c.execute("SELECT id  FROM users WHERE username = (%s)",
                  (thwart(self.username),))

    def createNewUser(self,username, password, email,):
        self.c.execute("INSERT INTO users (username, password, email, tracking) VALUES (%s, %s, %s, %s)",
                  (thwart(self.username), thwart(password), thwart(email), thwart("/introduction-to-python-programming/")))

    def getUserWikilinks(self):
        userId = self.getUserId()
        userLinks = self.c.execute("SELECT wikiLinks FROM userWikiLinks WHERE userId=(%s)",
                              (thwart(userId),))
        return userLinks

    def checkLinkInGlobalWikiList(self):
        isItThere = self.c.execute("SELECT id FROM wikiList WHERE article=(%s) ",
    ...
    def getWikiLocation(self):
        location = self.c.execute("SELECT location FROM wikiList WHERE article=(%s)",
                             (thwart(self.wikiArticleFullLink),))
        if location is None:
            return False
        return location
    ...
    def saveToGlobalWikiList(self):
        ...
        try:
            self.c.execute("INSERT INTO wikiList (location, article) VALUES (%s, %s) ",
